...
function diana_replace_author_content($content) {
    $author_name = __('Author: ', 'ddpro').get_the_author_meta( 'display_name', get_query_var( 'author' ) );
...
function diana_replace_author_content_two($content) {
    $author_name = get_the_author_meta( 'display_name', get_query_var( 'author' ) ).__('\'s Latest Posts', 'ddpro');
...
if (get_option(ddp_author_page_template) === 'global') $title_author = 'Global Archive - Row w Sidebar - PHP Templ - Diana';
